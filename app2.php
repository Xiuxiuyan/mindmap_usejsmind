...
<body>
    ...
    <div class="SideBar">
        <h2><a href="filelist.php">myFile</a></h2>
        <ul>
          <?php if(isset($filename["0"])):?>
           <li ><a href="#" onClick="showMyfile(<?php echo $filename["0"]?>"><?php echo $filename["0"];?></a></li>
          <?php endif;?>
          <?php if(isset($filename["1"])):?>
           <li onclick="showMyfile(<?php echo $filename["1"]?>)"><?php echo $filename['1'];?></li>
          <?php endif;?>
        </ul>
        <ul>
        	<li>新建思维导图
           ...
           <ul id="productsMenu" class="menu">
               <li>
                    ...
                    <ul id="newPhonesMenu" class="submenu">
                        ...
                        <li><a href="#" title="邀请加入协同" onClick="invite('<?php echo $value?>')"><?php echo $value;?></a></li>
                         <?php endforeach;?>
                    </ul>
               </li>
               <li>
...
<script type="text/javascript">
    ...
    $(function() {
    	...
    	var socket = new WebSocket('ws://localhost:80/webSocket/websocket');
    	...
    	$('#send').on('click', function() {
        	...
        	var name = map_json.meta.name;
        	var all = JSON.stringify(map_json);
            var username=<?php echo $_SESSION("name");?>;//用户名
            <?php $link=mysqli_connect("localhost","root","","web");//链接数据库
            if($link){
                $str="select fid from cooperation where uid="."'"."$name"."'";
                $result=mysqli_query($link,$str);
                $inv=mysqli_fetch_row($result);
                $invitor=$inv[0];
            } ?>
            var groupid = <?php echo $invitor;?>;//用户组id
    		// 发送消息
    ...
    //显示用户的文件
    function showMyfile(filename){
        if(!filename){
            alert("没有这个文件")
        }else{
            <?php 
                $fname=$_GET["filename"];
                $str="select content from file where fname="."'"."$fname"."'";
                $result=mysqli_query($link,$str);
                $cont=mysqli_fetch_row($result);
                $content=$cont[0]["content"];
            ?>
            mind = JSON.parse(<?php echo $content;?>);
            jm.show(mind);
        }
