...
var preparentid;//拖动前的父节点id
var nowparentid;//拖动后的父节点id
...
(function($w){
    ...
    jsMind.draggable.prototype = {
        ...
        dragstart:function(e){
            ...
            preparentid = jm.get_selected_node().parent.id;
        ...
        dragend:function(e){
            ...
            nowparentid = jm.get_selected_node().parent.id;
             if(preparentid == nowparentid)
                console.log("=");
            else{   //父节点改变
                console.log("!=");
                var fn = $("#transmit").text();
                var dbmindStr = showMap(fn);//从数据库里取出思维导图
                dbmind = JSON.parse(dbmindStr);
                console.log(dbmindStr);
                
                for(var i=0; i<dbmind.data.length; i++){
                    if(dbmind.data[i].id == jm.get_selected_node().id){//找到拖拽的节点
                        for(var k=0; k<dbmind.data.length; k++)
                            if(dbmind.data[k].id == nowparentid){//找到改变后的父节点
                                if(dbmind.data[k].parent.id != dbmind.data[i].id){
                                    dbmind.data[i].parentid = nowparentid;
                                    break;
                                }
                ...
                var mind_str = JSON.stringify(dbmind);
                console.log(mind_str);
                window.location.href="./save.php?content="+mind_str+"&filename="+fn;
            }
        },

        move_node:function(src_node,target_node,target_direct){
        	gbfjdid = target_node.id;
